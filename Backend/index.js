...
async function getValidAccessToken(req) {
  ...
  if (!refreshToken) throw new Error("Access token expired and no refresh token available. Please sign in again.");
  const resp = await axios.post("https://oauth2.googleapis.com/token", {
    client_id:     process.env.GOOGLE_CLIENT_ID,
    client_secret: process.env.GOOGLE_CLIENT_SECRET,
...
async function notifyAdminNewUser({ name, email, method }) {
  const adminEmail = "danushri.prakashsaranya@gmail.com";
      ...
      html: `
        <div style="font-family:-apple-system,sans-serif;max-width:480px;margin:0 auto;padding:28px;background:#f7f6f4;border-radius:12px;">
          ...
          <table style="width:100%;border-collapse:collapse;">
            <tr><td style="padding:10px 14px;background:#fff;border:1px solid #e5e2dd;border-radius:8px 8px 0 0;font-size:12px;color:#6b6560;font-weight:600;text-transform:uppercase;">Name</td><td style="padding:10px 14px;background:#fff;border:1px solid #e5e2dd;border-top:none;font-size:14px;font-weight:600;color:#1a1916;">${name}</td></tr>
            <tr><td style="padding:10px 14px;background:#f7f6f4;border:1px solid #e5e2dd;border-top:none;font-size:12px;color:#6b6560;font-weight:600;text-transform:uppercase;">Email</td><td style="padding:10px 14px;background:#f7f6f4;border:1px solid #e5e2dd;border-top:none;font-size:14px;color:#0d9488;font-weight:600;">${email}</td></tr>
            <tr><td style="padding:10px 14px;background:#fff;border:1px solid #e5e2dd;border-top:none;font-size:12px;color:#6b6560;font-weight:600;text-transform:uppercase;">Method</td><td style="padding:10px 14px;background:#fff;border:1px solid #e5e2dd;border-top:none;font-size:14px;color:#1a1916;">${method}</td></tr>
          </table>
...
// ── FIXED: isAfterHours — weekends are ALWAYS after hours ─────────────────────
function isAfterHours(dateTimeStr, workStart = 9, workEnd = 17) {
  if (!dateTimeStr) return false;
  const d   = new Date(dateTimeStr);
  const day = d.getDay(); // 0 = Sun, 6 = Sat
  if (day === 0 || day === 6) return true; // weekends always after-hours
  const h = d.getHours();
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    req.session.emailUser = {
      ...
      calendarLinked: user.calendarLinked, authMethod: "email",
    };
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    req.session.emailUser = {
      id: user._id.toString(), name: user.name, email: user.email,
      calendarLinked, authMethod: "email",
    };
    const token = jwt.sign(
      { id: user._id.toString(), email: user.email, name: user.name },
      JWT_SECRET, { expiresIn: "7d" }
  ...
  async (req, res) => {
    try {
      ...
      if (emailUserId) {
        ...
        req.session.emailUser = {
          id: emailUserId, name: dbUser?.name || "", email: dbUser?.email || "",
          googleEmail, calendarLinked: true, authMethod: "email",
        };
      }
      res.redirect((process.env.FRONTEND_URL || "http://localhost:5173") + "/home");
    } catch (err) {
  ...
  try {
    ...
    try {
      ...
      if (tokenErr.message === "CALENDAR_NOT_LINKED")
        return res.status(403).json({ error: "Calendar not connected. Please connect your Google Calendar.", calendarNotLinked: true });
      return res.status(401).json({ error: "Session expired. Please sign in again.", reauth: true });
    }
    ...
    const gcalResp = await axios.get(
      ...
      { headers: { Authorization: `Bearer ${token}` }, params: { timeMin, timeMax, singleEvents: true, orderBy: "startTime" } }
    );
    ...
    const allEvents = [
      ...events,
      ...outlookEvents.filter(oe => !events.some(ge =>
        ge.summary === oe.summary && ge.start?.dateTime?.slice(0,16) === oe.start?.dateTime?.slice(0,16)
      )),
...
app.get("/api/important-events", ensureAuth, async (req, res) => {
  try {
    ...
    const query = { email, status: { $nin: ["Completed", "Declined"] } };
    if (!includeOrganized) {
      query.$or = [
    ...
    const filtered = includeWeekends ? events : events.filter(e => !isWeekend(e.start?.dateTime));
    res.json(filtered);
  } catch (err) {
...
// ── After-hours events — FIXED ─────────────────────────────────────────────────
// Weekends are always after-hours. Weekdays use workStart/workEnd window.
app.get("/api/after-hours-events", ensureAuth, async (req, res) => {
  try {
    ...
    const all = await ImportantEvent.find({
      email,
      status: { $nin: ["Completed", "Declined"] },
    }).sort({ "start.dateTime": 1 });

    // isAfterHours now handles weekends correctly — see helper above
...
app.post("/api/user-settings", ensureAuth, async (req, res) => {
  try {
    const email = getUserEmail(req);
    const { workStart, workEnd, alertRange, includeWeekends, onlyIfInToList, keywords, emailEnabled, emailAddress, alertIntervals } = req.body;
    const settings = await UserSetting.findOneAndUpdate(
      { email },
      { workStart, workEnd, alertRange, includeWeekends, onlyIfInToList, keywords, emailEnabled, emailAddress, alertIntervals },
...
app.get("/api/profile/stats", ensureAuth, async (req, res) => {
  try {
    ...
    res.json({ totalImportant, totalEvents, afterHours: afterHoursCount, memberSince: dbUser?.createdAt || null });
  } catch (err) {
...
app.post("/api/accept-event/:eventId", ensureAuth, async (req, res) => {
  try {
    ...
    await axios.patch(
      ...
      { attendees: undefined },
      { headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, params: { sendUpdates: "none" } }
    );
    res.json({ ok: true });
  } catch (err) {
...
app.get("/api/debug/me", ensureAuth, async (req, res) => {
  ...
  res.json({
    sessionEmail: email, sessionId: id, dbFound: !!dbUser,
    calendarLinked: dbUser?.calendarLinked, hasAccessToken: !!dbUser?.accessToken, name: dbUser?.name,
  });
});
...
app.get("/api/debug/important-raw", ensureAuth, async (req, res) => {
  ...
  res.json({ count: all.length, events: all.map(e => ({
    ...
    importanceReason: e.importanceReason, importanceReasons: e.importanceReasons,
    youAreAnAttendee: e.youAreAnAttendee, start: e.start?.dateTime,
  }))});
});
...
app.get("/api/debug/send-raw-email", async (req, res) => {
  ...
  if (!user || !pass) return res.status(500).json({ error: "EMAIL_USER or EMAIL_PASS not set in .env" });
  try {
    const nodemailer = (await import("nodemailer")).default;
    const t = nodemailer.createTransport({ service: "gmail", auth: { user, pass } });
    await t.sendMail({ from: user, to: "danushri.prakashsaranya@gmail.com", subject: "WMW test email", text: "If you got this, email is working." });
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family:-apple-system,sans-serif;max-width:520px;margin:0 auto;padding:32px;background:#f7f6f4;border-radius:12px;">
          ...
          <p style="font-size:14px;color:#6b6560;line-height:1.7;">Your email alert setup is confirmed. You'll receive alerts at your chosen intervals (60, 30, and 15 minutes) before any important meetings.</p>
...
app.get("/api/auth/outlook", (req, res) => {
  ...
  const url = `https://login.microsoftonline.com/common/oauth2/v2.0/authorize?`
    ...
    + `&response_mode=query&prompt=consent&state=${state}`;
  res.redirect(url);
});
...
app.get("/api/auth/outlook/callback", async (req, res) => {
  ...
  try {
    const tokenResp = await axios.post(
      ...
      new URLSearchParams({ client_id: AZURE_CLIENT_ID, client_secret: AZURE_CLIENT_SECRET, code, redirect_uri: AZURE_REDIRECT_URI, grant_type: "authorization_code" }),
      { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
    );
    const { access_token, refresh_token } = tokenResp.data;
    const profileResp = await axios.get("https://graph.microsoft.com/v1.0/me", { headers: { Authorization: `Bearer ${access_token}` } });
    const msEmail = (profileResp.data.mail || profileResp.data.userPrincipalName || "").toLowerCase();
    let userId = null;
    try { const decoded = JSON.parse(Buffer.from(state, "base64").toString()); userId = decoded.uid; } catch (_) {}
    if (userId) {
      await User.findByIdAndUpdate(userId, { outlookAccessToken: access_token, outlookRefreshToken: refresh_token, outlookTokenIssuedAt: Date.now(), outlookEmail: msEmail, outlookLinked: true });
    } else if (req.user?.profile?.emails?.[0]?.value) {
      const googleEmail = req.user.profile.emails[0].value.toLowerCase();
      await User.findOneAndUpdate({ email: googleEmail }, { outlookAccessToken: access_token, outlookRefreshToken: refresh_token, outlookTokenIssuedAt: Date.now(), outlookEmail: msEmail, outlookLinked: true });
    }
...
app.post("/api/auth/outlook/disconnect", ensureAuth, async (req, res) => {
  try {
    const userEmail = getUserEmail(req);
    const query = req.session?.emailUser?.id ? { _id: req.session.emailUser.id } : { email: userEmail };
    await User.findOneAndUpdate(query, { outlookAccessToken: null, outlookRefreshToken: null, outlookTokenIssuedAt: null, outlookEmail: null, outlookLinked: false });
    res.json({ ok: true });
  } catch (err) {
...
async function getOutlookAccessToken(user) {
  ...
  const resp = await axios.post(
    "https://login.microsoftonline.com/common/oauth2/v2.0/token",
    new URLSearchParams({ client_id: AZURE_CLIENT_ID, client_secret: AZURE_CLIENT_SECRET, refresh_token: user.outlookRefreshToken, grant_type: "refresh_token", scope: MS_SCOPES }),
    { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
  );
  const newToken = resp.data.access_token;
  await User.findByIdAndUpdate(user._id, { outlookAccessToken: newToken, outlookTokenIssuedAt: Date.now(), ...(resp.data.refresh_token ? { outlookRefreshToken: resp.data.refresh_token } : {}) });
  return newToken;
}
...
async function fetchOutlookEvents(user, timeMin, timeMax) {
  ...
  return (resp.data.value || []).map(e => ({
    id: e.id, summary: e.subject || "Untitled",
    ...
    attendees: (e.attendees || []).map(a => ({
      ...
      responseStatus: a.status?.response === "accepted" ? "accepted" : a.status?.response === "tentatively" ? "tentative" : a.status?.response === "declined" ? "declined" : "needsAction",
    })),
...
app.get("/api/push/vapid-public-key", (req, res) => {
  if (!process.env.VAPID_PUBLIC_KEY) return res.status(400).json({ error: "Push notifications not configured." });
...
app.post("/api/push/test", ensureAuth, async (req, res) => {
  try {
    ...
    const payload = JSON.stringify({ title: "⏰ Wake Me When — Test Alarm", body: "Alarm notifications are working! You'll be woken up before important meetings.", tag: "wmw-test" });
